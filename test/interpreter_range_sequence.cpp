...
TEST_CASE("SimpleRange") {
  ...
  for (int i = 0; i < 5; i++) {
    CHECK(seq_value->fields[i]->type == yona::interp::runtime::Int);
...
TEST_CASE("SingleElementRange") {
  ...
  CHECK(seq_value->fields[0]->type == yona::interp::runtime::Int);
...
TEST_CASE("FloatRange") {
  ...
  for (int i = 0; i < 4; i++) {
    CHECK(seq_value->fields[i]->type == yona::interp::runtime::Float);
