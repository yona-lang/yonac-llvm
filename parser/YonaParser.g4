...
bodyWithGuards
  : NEWLINE? VLINE guard = expression OP_ASSIGN NEWLINE? expr = expression NEWLINE;
bodyWithoutGuard: NEWLINE? OP_ASSIGN NEWLINE? expression NEWLINE;

expression
  : PARENS_L expression PARENS_R                                                               # expressionInParents
  ...
  ) right = expression                                                                           # binaryShiftExpression
...
conditional
  : KW_IF ifX = expression NEWLINE? KW_THEN NEWLINE? thenX = expression NEWLINE? KW_ELSE NEWLINE? elseX = expression;
apply: call funArg*;
...
stringLiteral
  : STRING_START (
    rawStringPart = REGULAR_STRING_INSIDE
    | interpolatedStringPart
  )* STRING_STOP;
interpolatedStringPart
  : OPEN_INTERP interpolationExpression = expression (
    COMMA alignment = expression
  )? CLOSE_INTERP;
...
dict
  : CURLY_L NEWLINE? (
    dictKey OP_ASSIGN dictVal (COMMA NEWLINE? dictKey OP_ASSIGN dictVal)*
  )? NEWLINE? CURLY_R;
dictKey: expression;
dictVal: expression;
sequence: emptySequence | otherSequence | rangeSequence;
set
  : CURLY_L NEWLINE? expression (COMMA NEWLINE? expression)* NEWLINE? CURLY_R;
...
otherSequence
  : BRACKET_L NEWLINE? expression (COMMA NEWLINE? expression)* NEWLINE? BRACKET_R;
rangeSequence
  : BRACKET_L NEWLINE? (step = expression NEWLINE? COMMA)? start = expression NEWLINE? DDOT NEWLINE? end = expression
      NEWLINE? BRACKET_R;

caseExpr
  : KW_CASE expression KW_OF NEWLINE? patternExpression+ NEWLINE? KW_END;
patternExpression
  : pattern (patternExpressionWithoutGuard | patternExpressionWithGuard+) NEWLINE;
...
patternExpressionWithGuard
  : NEWLINE? VLINE guard = expression OP_RIGHT_ARROW NEWLINE? expr = expression;

pattern
  : PARENS_L pattern PARENS_R
  | underscore
  | patternValue
...
asDataStructurePattern
  : identifier AT (
    PARENS_L dataStructurePattern PARENS_R
    | dataStructurePattern
  );

patternWithoutSequence
  : underscore
  | patternValue
  | tuplePattern
  | dictPattern;
...
sequencePattern
  : BRACKET_L (pattern (COMMA pattern)*)? BRACKET_R
  | headTails
  | tailsHead
  | headTailsHead;
...
dictPattern
  : CURLY_L (
    patternValue OP_ASSIGN pattern (COMMA patternValue OP_ASSIGN pattern)*
  )? CURLY_R;

recordPattern
  : recordType (
    PARENS_L (name OP_ASSIGN pattern) (COMMA name OP_ASSIGN pattern)* PARENS_R
  )?;

importExpr
  : KW_IMPORT NEWLINE? (importClause NEWLINE?)+ KW_IN NEWLINE? expression;
...
catchPatternExpression
  : (triplePattern | underscore) (
    catchPatternExpressionWithoutGuard
    | catchPatternExpressionWithGuard+
  ) NEWLINE;
triplePattern: PARENS_L pattern COMMA pattern COMMA pattern PARENS_R;
catchPatternExpressionWithoutGuard
  : NEWLINE? OP_RIGHT_ARROW NEWLINE? expression;
catchPatternExpressionWithGuard
  : NEWLINE? VLINE guard = expression OP_RIGHT_ARROW NEWLINE? expr = expression;
...
withExpr
  : KW_WITH KW_DAEMON? context = expression (KW_AS name)? NEWLINE? body = expression NEWLINE? KW_END;

generatorExpr: sequenceGeneratorExpr | setGeneratorExpr | dictGeneratorExpr;
sequenceGeneratorExpr
  : BRACKET_L reducer = expression VLINE collectionExtractor OP_LEFT_ARROW stepExpression = expression NEWLINE? (
    KW_IF condition = expression
  )? BRACKET_R;
setGeneratorExpr
  : CURLY_L reducer = expression VLINE collectionExtractor OP_LEFT_ARROW stepExpression = expression NEWLINE? (
    KW_IF condition = expression
  )? CURLY_R;
dictGeneratorExpr
  : CURLY_L dictGeneratorReducer VLINE collectionExtractor OP_LEFT_ARROW stepExpression = expression NEWLINE? (
    KW_IF condition = expression
  )? CURLY_R;
...
keyValueCollectionExtractor
  : key = identifierOrUnderscore OP_ASSIGN val = identifierOrUnderscore;
...
recordInstance
  : recordType PARENS_L NEWLINE? (name OP_ASSIGN expression) (
