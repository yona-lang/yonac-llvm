...
**Memory Management**: Atomic reference counting with recursive destructors and hybrid Perceus. See `docs/memory-management.md` for full details. Key points: All heap objects use `rc_alloc` with a `[refcount, type_tag_encoded]` header. Atomic `rc_inc`/`rc_dec` (`__atomic_fetch_add/sub`). Recursive destructors free children via `heap_mask` bitmasks (closures, ADTs, tuples, seqs, sets, dicts). Hybrid Perceus: non-seq types (closures, strings, ADTs, tuples) use callee-owns DUP/DROP at call sites; seqs use callee-borrows with unique-owner in-place optimization. Weak self-references break recursive closure cycles. io_uring buffer pinning for async I/O safety. Escape analysis (`include/EscapeAnalysis.h`) for arena allocation of non-escaping values.
...
- Platform-specific runtime in `src/runtime/platform/` — `uring.h` (shared io_uring), `file_linux.c`, `net_linux.c`, `os_linux.c`
