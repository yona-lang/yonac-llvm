...
**Visitor pattern with generic return types**: `AstVisitor<ResultType>` is a templated base class. The Interpreter uses `AstVisitor<InterpreterResult>`. AST nodes implement `accept()` that dispatches to the correct `visit()` overload.
...
**Promise-aware type system**: `PromiseType` in `types.h` wraps an inner type. `Promise<T>` auto-coerces to `T` — the runtime auto-awaits where needed without user-visible `async`/`await` keywords. Native async functions are marked with `is_async` flag and return promises that are auto-awaited at binary operators, function arguments, conditions, and pattern match targets.
...
- **Type System** (`include/types.h`): Variant-based types including builtins, function types, product/sum types, record types, and named types. Used by the interpreter runtime; the codegen uses its own `CType` + `TypedValue` system with compile-time type inference.
