...
**Memory Management**: Reference counting with arena optimization. All heap objects (`SEQ`, `SET`, `DICT`, `ADT`, `FUNCTION`, `STRING`, `BYTES`) use `rc_alloc` which prepends a `[refcount, type_tag]` header. Let bindings `rc_inc` the result and `rc_dec` bindings at scope exit. Closures `rc_inc` captured values. Function parameters use callee-borrows convention (no RC on params). Escape analysis (`include/EscapeAnalysis.h`) determines which let-bound values don't escape their scope; non-escaping values are bump-allocated from a per-scope arena (`yona_rt_arena_alloc`) with sentinel refcount (`INT64_MAX`) so `rc_dec` safely skips them, and the entire arena is freed in bulk at scope exit.
