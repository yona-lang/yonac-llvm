...
```yona
import alloc from Std\Bytes in
...
```yona
import length from Std\Bytes in
...
```yona
import get, fromString from Std\Bytes in
...
```yona
import alloc, set, get from Std\Bytes in
let buf = alloc 4 in
do
  set buf 0 42
  get buf 0   # => 42
...
```yona
import concat, fromString from Std\Bytes in
...
```yona
import slice, fromString, toString from Std\Bytes in
...
```yona
import fromString from Std\Bytes in
...
```yona
import fromString, toString from Std\Bytes in
...
```yona
import fromSeq, get from Std\Bytes in
...
```yona
import fromString, toSeq from Std\Bytes in
