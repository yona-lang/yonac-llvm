...
private:
    ...
    unique_ptr<ModuleExpr> parse_module_internal() {
        ...
        if (!expect(TokenType::MODULE, "Expected 'module' at start of file")) {
            return nullptr;
        }
        ...
        expect(TokenType::AS, "Expected 'as' after module header");
        ...
        expect(TokenType::END, "Expected 'end' at end of module");
    ...
    unique_ptr<PackageNameExpr> parse_module_name() {
        ...
        } while (match(TokenType::DOT)); // Use dot for module path separator
    ...
    parse_function_clause_with_patterns(const string& expected_name) {
        ...
        if (!check(TokenType::IDENTIFIER) || peek().lexeme != expected_name) {
            error(ParseError::Type::INVALID_SYNTAX, 
        ...
        // Parse patterns
        vector<PatternNode*> patterns;
        while (!check(TokenType::ASSIGN) && !check(TokenType::PIPE) && !is_at_end()) {
        ...
        if (match(TokenType::PIPE)) {
            ...
            do {
                ...
                expect(TokenType::ASSIGN, "Expected '=' after guard");
    ...
    unique_ptr<compiler::types::Type> parse_product_type() {
        if (check(TokenType::LPAREN)) {
            ...
            if (!expect(TokenType::RPAREN, "Expected ')' after tuple types")) {
                return nullptr;
            }
    ...
    unique_ptr<compiler::types::Type> parse_collection_type() {
        if (check(TokenType::LBRACKET)) {
            ...
            if (!expect(TokenType::RBRACKET, "Expected ']' after sequence type")) {
                return nullptr;
            }
        ...
        if (check(TokenType::LBRACE)) {
            ...
            if (match(TokenType::COLON)) {
                ...
                if (!expect(TokenType::RBRACE, "Expected '}' after dict type")) {
                    return nullptr;
                }
                ...
                // Set type: {Type}
                if (!expect(TokenType::RBRACE, "Expected '}' after set type")) {
                    return nullptr;
                }
    ...
    unique_ptr<RecordNode> parse_record_definition() {
        ...
        if (!expect(TokenType::LPAREN, "Expected '(' after record name")) {
            return nullptr;
        }
        ...
        if (!check(TokenType::RPAREN)) {
            do {
                ...
                // Expect colon
                if (!expect(TokenType::COLON, "Expected ':' after field name")) {
                    return nullptr;
                }
        ...
        if (!expect(TokenType::RPAREN, "Expected ')' after record fields")) {
            return nullptr;
        }
    ...
    unique_ptr<TypeDefinition> parse_type_definition() {
        ...
        while (check(TokenType::IDENTIFIER) && !check_ahead(TokenType::ASSIGN)) {
            type_params.push_back(string(current().lexeme));
            advance();
        }
        
        if (!expect(TokenType::ASSIGN, "Expected '=' in type definition")) {
            return nullptr;
        }
    ...
    unique_ptr<PatternNode> parse_pattern_primary() {
        ...
        if (check(TokenType::INTEGER) || check(TokenType::FLOAT) || 
            check(TokenType::STRING) || check(TokenType::CHARACTER) ||
        ...
        if (match(TokenType::LPAREN)) {
            ...
            expect(TokenType::RPAREN, "Expected ')' after tuple pattern");
        ...
        if (match(TokenType::LBRACKET)) {
            ...
            if (!check(TokenType::RBRACKET)) {
                ...
                while (true) {
                    ...
                    if (check(TokenType::PIPE)) {
                        ...
                        expect(TokenType::RBRACKET, "Expected ']' after tail pattern");
            ...
            expect(TokenType::RBRACKET, "Expected ']' after list pattern");
        ...
        if (match(TokenType::LBRACE)) {
            ...
            expect(TokenType::RBRACE, "Expected '}' after record pattern");
    ...
    bool could_start_expr(TokenType type) const {
        switch (type) {
            ...
            case TokenType::BIT_NOT:  // unary complement
                return true;
            default:
                return false;
        }
    ...
    unique_ptr<ExprNode> parse_juxtaposition_apply(unique_ptr<ExprNode> func) {
        ...
        if (auto id = dynamic_cast<IdentifierExpr*>(func.get())) {
            ...
            BOOST_LOG_TRIVIAL(error) << "parse_juxtaposition_apply: Non-identifier function application not yet supported";
    ...
    unique_ptr<ExprNode> parse_expr(Precedence min_prec = Precedence::LOWEST) {
        ...
        if (recursion_depth > 50) {
            BOOST_LOG_TRIVIAL(error) << "parse_expr: Max recursion depth exceeded!";
        ...
        while (!is_at_end()) {
            if (++loop_count > 100) {
                BOOST_LOG_TRIVIAL(error) << "parse_expr: Infinite loop detected at position " << current_ 
                                        << "/" << tokens_.size()
                                        << ", token: " << (current_ < tokens_.size() ? peek().lexeme : "EOF")
                                        << ", type: " << (current_ < tokens_.size() ? static_cast<int>(peek().type) : -1);
                // Print surrounding tokens for context
                if (current_ > 0 && current_ - 1 < tokens_.size()) {
                    BOOST_LOG_TRIVIAL(error) << "  Previous token: " << tokens_[current_ - 1].lexeme;
                }
                if (current_ + 1 < tokens_.size()) {
                    BOOST_LOG_TRIVIAL(error) << "  Next token: " << tokens_[current_ + 1].lexeme;
                }
    ...
    unique_ptr<ExprNode> parse_expr_until_in() {
        ...
        while (!is_at_end() && peek().type != TokenType::IN) {
            if (++loop_count > 1000) {
                BOOST_LOG_TRIVIAL(error) << "parse_expr_until_in: Infinite loop detected!";
        ...
        switch (peek().type) {
            ...
            case TokenType::LBRACKET: {
                ...
                if (match(TokenType::PIPE)) {
                    ...
                    expect(TokenType::RBRACKET, "Expected ']' after range");
            ...
            case TokenType::LBRACE: {
                ...
                if (match(TokenType::COLON)) {
                    ...
                    expect(TokenType::RBRACE, "Expected '}' after set");
...
#define BINARY_OP_RIGHT_ASSOC(TokenType, ExprType, OpStr) \
        ...
        switch (op.type) {
            // Binary operators
            BINARY_OP(TokenType::PLUS, AddExpr, "+")
            BINARY_OP(TokenType::MINUS, SubtractExpr, "-")
            BINARY_OP(TokenType::STAR, MultiplyExpr, "*")
            BINARY_OP(TokenType::SLASH, DivideExpr, "/")
            BINARY_OP(TokenType::PERCENT, ModuloExpr, "%")
            BINARY_OP_RIGHT_ASSOC(TokenType::POWER, PowerExpr, "**")
                    
            // Comparison
            BINARY_OP(TokenType::EQ, EqExpr, "==")
            BINARY_OP(TokenType::NEQ, NeqExpr, "!=")
            BINARY_OP(TokenType::LT, LtExpr, "<")
            BINARY_OP(TokenType::GT, GtExpr, ">")
            BINARY_OP(TokenType::LTE, LteExpr, "<=")
            BINARY_OP(TokenType::GTE, GteExpr, ">=")
                    
            // Logical
            BINARY_OP(TokenType::AND, LogicalAndExpr, "&&")
            BINARY_OP(TokenType::OR, LogicalOrExpr, "||")
                    
            // Bitwise
            BINARY_OP(TokenType::BIT_AND, BitwiseAndExpr, "&")
            BINARY_OP(TokenType::PIPE, BitwiseOrExpr, "|")
            BINARY_OP(TokenType::BIT_XOR, BitwiseXorExpr, "^")
            BINARY_OP(TokenType::LEFT_SHIFT, LeftShiftExpr, "<<")
            BINARY_OP(TokenType::RIGHT_SHIFT, RightShiftExpr, ">>")
            BINARY_OP(TokenType::ZERO_FILL_RIGHT_SHIFT, ZerofillRightShiftExpr, ">>>")
                    
            // Cons operators
            BINARY_OP_RIGHT_ASSOC(TokenType::CONS, ConsLeftExpr, "::")
                    
            // Pipe operators
            BINARY_OP_RIGHT_ASSOC(TokenType::PIPE_LEFT, PipeLeftExpr, "<|")
            BINARY_OP(TokenType::PIPE_RIGHT, PipeRightExpr, "|>")
            BINARY_OP(TokenType::JOIN, JoinExpr, "++")
                    
            // Membership
            BINARY_OP(TokenType::IN, InExpr, "in")
                    
            // TODO: Implement actual cons operators :: and :>
            // For now, these are commented out to avoid duplicate case labels
            /*case TokenType::CONS_RIGHT:
                return make_unique<ConsRightExpr>(loc, left.release(),
                    parse_expr(next_precedence(prec)).release());
                    
            case TokenType::CONS_LEFT:
    ...
    Precedence get_infix_precedence(TokenType type) const {
        switch (type) {
            // Pipe operators
            case TokenType::PIPE_LEFT:
            case TokenType::PIPE_RIGHT:
    ...
    unique_ptr<ExprNode> parse_if_expr() {
        ...
        expect(TokenType::ELSE, "Expected 'else' after then expression");
    ...
    unique_ptr<AliasExpr> parse_alias() {
        ...
        expect(TokenType::ASSIGN, "Expected '=' in let binding");
    ...
    unique_ptr<ExprNode> parse_case_expr() {
        ...
        auto expr = parse_expr();
        expect(TokenType::OF, "Expected 'of' after case expression");
        ...
        expect(TokenType::END, "Expected 'end' after case patterns");
    ...
    unique_ptr<CaseClause> parse_case_clause() {
        ...
        expect(TokenType::ARROW, "Expected '->' after pattern");
    ...
    unique_ptr<ExprNode> parse_do_expr() {
        ...
        expect(TokenType::END, "Expected 'end' after do block");
    ...
    unique_ptr<ExprNode> parse_try_expr() {
        ...
        while (match(TokenType::CATCH)) {
            ...
            expect(TokenType::ARROW, "Expected '->' after catch pattern");
    ...
    unique_ptr<ExprNode> parse_with_expr() {
        ...
        expect(TokenType::AS, "Expected 'as' in with expression");
        ...
        string name(advance().lexeme);
        expect(TokenType::DO, "Expected 'do' in with expression");
    ...
    unique_ptr<ExprNode> parse_lambda_expr(bool stop_at_in = false) {
        ...
        // Expect opening parenthesis
        if (!expect(TokenType::LPAREN, "Expected '(' after '\\' in lambda expression")) {
            return nullptr;
        }
        ...
        if (!check(TokenType::RPAREN)) {
            do {
                ...
                if (param) params.push_back(param.release());
                    ...
                    BOOST_LOG_TRIVIAL(error) << "parse_lambda_expr: Failed to parse parameter";
                    return nullptr;
                }
            } while (match(TokenType::COMMA));
        }
        ...
        // Expect closing parenthesis
        if (!expect(TokenType::RPAREN, "Expected ')' after lambda parameters")) {
            return nullptr;
        }
    ...
    unique_ptr<ExprNode> parse_sequence_generator(SourceLocation loc, unique_ptr<ExprNode> expr) {
        ...
        expect(TokenType::RBRACKET, "Expected ']' after sequence generator");
    ...
    unique_ptr<ExprNode> parse_set_generator(SourceLocation loc, unique_ptr<ExprNode> expr) {
        ...
        expect(TokenType::RBRACE, "Expected '}' after set generator");
    ...
    unique_ptr<CollectionExtractorExpr> parse_collection_extractor() {
        ...
        auto pattern = parse_pattern();
        expect(TokenType::ASSIGN, "Expected '<-' in generator");
        auto collection = parse_expr();
        
        ExprNode* condition = nullptr;
        if (match(TokenType::COMMA) && check(TokenType::IF)) {
            advance(); // consume 'if'
    ...
    unique_ptr<ExprNode> parse_dict(SourceLocation loc, unique_ptr<ExprNode> first_key) {
        ...
        if (match(TokenType::PIPE)) {
            ...
            expect(TokenType::RBRACE, "Expected '}' after dict generator");
        ...
        while (match(TokenType::COMMA)) {
            auto key = parse_expr();
            expect(TokenType::COLON, "Expected ':' in dictionary entry");
    ...
    unique_ptr<ExprNode> parse_import_expr() {
        ...
        if (!expect(TokenType::IN, "Expected 'in' after import clauses")) {
            return nullptr;
        }
