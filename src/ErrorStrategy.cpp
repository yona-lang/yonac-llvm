...
void ErrorStrategy::handleRecognitionException(IntervalSet et, const string &message, const exception &cause, int line) {
  // TODO
...
void ErrorStrategy::reportNoViableAlternative(Parser *parser, const NoViableAltException &e) {
  ...
  ast_ctx.addError(yona_error(SourceInfo(*offendingToken, *recognizer), yona_error::Type::SYNTAX, msg));

  recognizer->getErrorListenerDispatch().syntaxError(recognizer, offendingToken, offendingToken->getLine(), offendingToken->getCharPositionInLine(),
                                                     msg, make_exception_ptr(e));
}
} // namespace yona::parser
