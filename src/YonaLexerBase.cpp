...
void YonaLexerBase::addSymbol() { symbols.insert(getText().substr(1)); }
...
unique_ptr<antlr4::CommonToken> YonaLexerBase::curlyToken(string const &text) {
  ...
  return make_unique<antlr4::CommonToken>(
      new antlr4::CommonToken(make_pair(this, getInputStream()), YonaLexer::REGULAR_STRING_INSIDE, DEFAULT_TOKEN_CHANNEL, start, stop));
}
