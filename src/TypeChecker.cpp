...
Type TypeChecker::visit(TypeDefinition *node) const {
    ...
    if (node->typeNames.empty()) {
        return Type(nullptr);
    }

    if (node->typeNames.size() == 1) {
        // Simple type alias
        return type_node_to_type(node->typeNames[0]);
    } else {
        // Sum type with multiple alternatives
        auto sum_type = make_shared<SumType>();
        // Note: SumType doesn't have a name field in the current implementation
        // We need to store it separately or modify SumType

        for (auto* type_node : node->typeNames) {
            Type alt_type = type_node_to_type(type_node);
            sum_type->types.insert(alt_type);
        }
